...
export default {
  ...
  methods: {
    ...
    handleMouseDown() {
      if (this.model) {
        this.model.autoInteract = true; // 鼠标左键按下时，启用autoInteract
      }
    },
    ...
    handleMouseUp() {
      if (this.model) {
        this.model.autoInteract = false; // 鼠标左键松开时，禁用autoInteract
      }
    },
...
.transparent-textarea {
  ...
  left: 40%; /* 水平居中 */
  transform: translateX(-40%); /* 调整以确保输入框完全居中 */
  padding: 10px;
  font-size: 20px;
...
.send-btn {
  position: absolute;
  bottom: 7.1%; /* 与输入框保持同一水平线 */
  left: 72%; /* 输入框宽度60% + 左边距50%转换后，取输入框右侧位置 */
  transform: translateX(-72%); /* 微调居中 */
  padding: 12px 24px;
  font-size: 20px;
  background-color: rgba(139, 235, 252, 0.9); /* 绿色系表示发送 */
  border: none;
  border-radius: 5px;
  color: rgb(0, 0, 0);
  cursor: pointer;
